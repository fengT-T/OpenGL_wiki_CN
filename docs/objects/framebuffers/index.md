...
| 术语 | 说明 |
|------|------|
| **Image（图像）** | 单个 2D 像素数组，具有特定的 [Image Format（图像格式）](/objects/textures/formats) |
...
层叠图像用于 Layered Rendering（分层渲染），可以将不同图元发送到帧缓冲区的不同层。
...
- OpenGL 4.5+ 或 ARB_texture_barrier 支持 Texture Barrier（纹理屏障），允许有限制的同像素读写
